...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--output-dir",
        default=os.environ.get("SURVEY_OUTPUT", "reports"),
...
def save_grouped_bar(df, x_col, col1, col2, label1, label2, title, fname, figsize=(10, 6)):
    ...
    fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, fname), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_rising_stars.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_declining_tech.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_jobsat_age.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_comp_exp_age.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_devtype_comp_sat.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_geo_heatmap.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_age_lang_stacked.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
if "Remote" in rd_pivot.columns:
    ...
    fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_remote_devtype.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_ai_sentiment.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
if cols_to_plot:
    ...
    for tick in ax.get_xticklabels(): tick.set_rotation(45); tick.set_ha("right")
    fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_ai_age.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_knowledge_corr.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_remote_comp_sat_matrix.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
fig.tight_layout(); fig.savefig(os.path.join(CHARTS_DIR, "chart_country_remote_comp.png"), dpi=150, bbox_inches="tight"); plt.close(fig)
...
L("> directory are produced by `src/generate_report.py` from the SQLite database")
L(f"> `{os.path.basename(DB_PATH)}`.")
L(">")
L("> - **Dataset:** Stack Overflow Developer Survey 2024 (ODbL). See `NOTICE.md`.")
L("> - **Pipeline:** `src/build_database.py` → `src/generate_report.py` (see `docs/methodology.md`).")
...
L(
    ...
    "correlational (see the Discussion and `docs/limitations.md`)."
)
L()
...
L()
L("![Languages Currently Used](charts/chart_lang_current.png)")
L()
...
L("#### Wanted Languages")
L()
L("![Languages Desired](charts/chart_lang_wanted.png)")
L()
...
L()
L("![Databases Currently Used](charts/chart_db_current.png)")
L()
...
L("#### Wanted Databases")
L()
L("![Databases Desired](charts/chart_db_wanted.png)")
L()
...
L()
L("![Platform Trends](charts/chart_platform_trends.png)")
L()
...
L()
L("![Rising Stars](charts/chart_rising_stars.png)")
L()
...
L()
L("![Declining Tech](charts/chart_declining_tech.png)")
L()
...
L()
L("![Job Satisfaction Distribution](charts/chart_jobsat_dist.png)")
L()
...
L()
L("![Satisfaction Factors](charts/chart_jobsat_factors.png)")
L()
...
L()
L("![Satisfaction by Remote](charts/chart_jobsat_remote.png)")
L()
...
L()
L("![Satisfaction by Dev Type](charts/chart_jobsat_devtype.png)")
L()
...
L()
L("![Satisfaction by Country](charts/chart_jobsat_country.png)")
L()
...
L()
L("![Satisfaction by Age](charts/chart_jobsat_age.png)")
L()
...
L()
L("![IC vs Manager](charts/chart_jobsat_icpm.png)")
L()
...
L()
L("![Compensation Distribution](charts/chart_comp_dist.png)")
L()
...
L()
L("![Compensation by Country](charts/chart_comp_country.png)")
L()
...
L()
L("![Compensation by Dev Type](charts/chart_comp_devtype.png)")
L()
...
L()
L("![Compensation by Education](charts/chart_comp_edlevel.png)")
L()
...
L()
L("![Compensation by Age](charts/chart_comp_age.png)")
L()
...
L()
L("![Compensation by Remote](charts/chart_comp_remote.png)")
L()
...
L()
L("![Experience vs Comp](charts/chart_comp_exp_age.png)")
L()
...
L()
L("![Dev Role Comp vs Sat](charts/chart_devtype_comp_sat.png)")
L()
...
L()
L("![Geographic Heatmap](charts/chart_geo_heatmap.png)")
L()
...
L()
L("![Age Language Stacked](charts/chart_age_lang_stacked.png)")
L()
...
L()
L("![Remote by Dev Type](charts/chart_remote_devtype.png)")
L()
...
L()
L("![AI Sentiment](charts/chart_ai_sentiment.png)")
L()
...
L()
L("![AI by Age Group](charts/chart_ai_age.png)")
L()
...
L()
L("![Knowledge Scores](charts/chart_knowledge.png)")
L()
...
L()
L("![Knowledge Correlations](charts/chart_knowledge_corr.png)")
L()
...
L()
L("![Learning Compensation](charts/chart_learn_comp.png)")
L()
...
L()
L("![Employment Compensation](charts/chart_emp_comp.png)")
L()
...
L()
L("![Python Migration](charts/chart_python_migration.png)")
L()
...
L()
L("![Remote Comp Sat Matrix](charts/chart_remote_comp_sat_matrix.png)")
L()
...
L()
L("![Country Remote Comp](charts/chart_country_remote_comp.png)")
L()
...
L()
L("![SO Visit Frequency](charts/chart_so_visit.png)")
L()
...
L()
L("![SO Participation Compensation](charts/chart_so_comp.png)")
L()
...
L()
L("![Employment Distribution](charts/chart_employment_dist.png)")
L()
...
L()
L("### Job Satisfaction")
L(f"The mean satisfaction score of approximately {jobsat_mean:.1f}/10 suggests moderate-to-high overall satisfaction. Among the individual satisfaction factors, **{sat_top}** and **{sat_second}** score highest on average, while **{sat_low}** scores lowest. (See docs/methodology.md for how the factor scores are derived.)")
L()
...
L()
L("7. **Work Patterns**: Remote work is associated with higher average satisfaction and higher median pay than in-person work, with hybrid work in between. These are descriptive correlations (see §4.6, §3.3, and docs/limitations.md).")
L()
L("---")
L()
